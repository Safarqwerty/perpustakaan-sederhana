    ...
    <div class="min-h-screen flex items-center justify-center">
        <div class="container mx-auto">
                    ...
                    @else
                        ...
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}"
                                class="font-semibold text-gray-600 hover:text-gray-900">Daftar</a>
                        @endif
                    @endauth
                </div>
            ...
            <div class="flex flex-col items-center mt-16">
                <h1 class="text-3xl font-semibold mb-6 text-center text-gray-800 dark:text-white">Selamat datang di
                    Sistem Informasi Perpustakaan Sekolah</h1>
                <h2 class="text-3xl font-semibold mb-6 text-red-400">{{ App\Models\Setting::first()->webname }}</h2>
                <p class="text-gray-100 text-center max-w-md text-gray-800 dark:text-white">Masuk untuk mengakses
                    layanan perpustakaan secara online.</p>
                <div class="flex flex-col md:flex-row mt-8 space-y-4 md:space-y-0 md:space-x-4">
                    <a href="{{ route('login') }}"
                        class="bg-white shadow-lg rounded-lg p-4 flex items-center justify-center space-x-2 transition duration-300 hover:shadow-xl w-full md:w-auto">
                    ...
                    <a href="{{ route('register') }}"
                        class="bg-white shadow-lg rounded-lg p-4 flex items-center justify-center space-x-2 mt-4 md:mt-0 transition duration-300 hover:shadow-xl w-full md:w-auto">
                        <div class="w-12 h-12 bg-red-50 rounded-full flex items-center justify-center">
            ...
            <div class="flex justify-between items-center mt-16">
                <p class="text-sm text-gray-500">© {{ date('Y') }} Sistem Informasi Perpustakaan Sekolah. <a
                        href="">Dibuat dengan ❤️</a></p>
                <p class="text-sm text-gray-500">Laravel v{{ Illuminate\Foundation\Application::VERSION }} (PHP
                    v{{ PHP_VERSION }})</p>
            </div>
        </div>
    </div>
